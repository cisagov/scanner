...
```console
docker run cisagov/scanner:1.3.1-rc.1
```
...
1. Create a `docker-compose.yml` file similar to the one below to use [Docker Compose](https://docs.docker.com/compose/).
    ...
    services:
      scanner:
        image: cisagov/scanner:1.3.1-rc.1
        volumes:
          - type: bind
            source: <your_log_dir>
...
1. Pull the new image:
    ...
    ```console
    docker pull cisagov/scanner:1.3.1-rc.1
    ```
...
|-----------|-------------|
|`cisagov/scanner:1.3.1-rc.1`| An exact release version. |
...
```console
docker build \
  --build-arg VERSION=1.3.1-rc.1 \
  --tag cisagov/scanner:1.3.1-rc.1 \
